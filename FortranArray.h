...
struct range {
    ...
    // static constructors
    static constexpr range  _(int, unsigned);
...
// tensor, dimension, and fdms_ synopsis

namespace fa {
namespace detail_dim_ {
template<char FC, class T, range::code_t... NN>
struct fdms_;
} // namespace detail_dim_

template<class T, r::code_t... NN>
class tensor : public detail_dim_::fdms_<'c', T, r::_0(NN)...> {};

template<class T, r::code_t... NN>
class dimension : public detail_dim_::fdms_<'c', T, r::_1(NN)...> {};
} // namespace fa
...
struct fdms_ {
    // index
    template<class... SS>
    static index_t c_index(SS...);
    template<class... SS>
    static index_t fortran_index(SS...);
...
struct allocatable {
    ...
    // index
    template<class... SS>
    index_t c_index(SS...) const;
    template<class... SS>
    index_t fortran_index(SS...) const;
...
private:
    ...
    struct range_traits_impl_;

    template<class T>
    struct range_traits_impl_<T, 32> {
    ...
    template<class T>
    struct range_traits_impl_<T, 64> {
    ...
    struct range_traits_ : public range_traits_impl_<int, TOTALBITS> {
        using base_t = range_traits_impl_<int, TOTALBITS>;
        ...
        unsigned explicit_ : base_t::e_bits;
        ...
        static constexpr utype check_size_(int isize)
        {
            ...
            return isize;
#else
            return (0 <= isize && isize <= base_t::max_size) ? isize
        ...
        static constexpr stype to_code_(stype iexplicit, stype ifront, utype isize)
        {
            return (iexplicit << (base_t::f_bits + base_t::size_bits))
                + (ifront << (base_t::e_bits + base_t::size_bits) >> base_t::e_bits) + isize;
        }
        ...
        {
            return code_if_explicit_(icode) ? icode : range_traits_(1, N, code_to_size_(icode)).to_code_();
        }
    };

#ifdef EIGEN_DEFAULT_DENSE_INDEX_TYPE
    static constexpr size_t nbit_ = 8 * sizeof(EIGEN_DEFAULT_DENSE_INDEX_TYPE);
    static_assert(sizeof(range_traits_<nbit_>) == sizeof(EIGEN_DEFAULT_DENSE_INDEX_TYPE), "");
#else
    // default platform information
    static constexpr size_t nbit_ = 8 * sizeof(size_t);
    static_assert(sizeof(range_traits_<nbit_>) == sizeof(size_t), "");
#endif

    using type = range_traits_<nbit_>;
...
struct D1<'f', N, NN...> {
    ...
    static index_t index(S s, SS... ss)
    {
        return s - range(N).front() + range(N).size() * D1<'f', NN...>::index(ss...);
    }
};
///@}
...
struct traits<'f', T, N, NN...> {
    using type  = typename traits<'f', typename traits<'f', T, N>::type, NN...>::type;
    using decay = typename std::decay<type>::type;
    using base  = typename std::remove_pointer<decay>::type;
};
///@}
 ...
 */
template<class T, r::code_t... NN>
class dimension : public detail_dim_::fdms_<'f', T, r::_1(NN)...> {};

namespace detail_alcb_ {
/**
...
struct ad {
    ...
    void reserve_impl(SS... ss)
    {
        ...
        copy_dims<detail_dim_::sanity<FC>::fc, sizeof...(BB), SS...>::exec(dims_, ss...);
        data_ = new T[size()];
    }
};
 ...
 */
template<class T, int... BEGINS>
class allocatable : private detail_alcb_::aimpl<T, BEGINS...> {
private:
    using impl_t = detail_alcb_::aimpl<T, BEGINS...>;
